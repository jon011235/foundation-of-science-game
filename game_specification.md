...
Not so much of an issue, but something to keep in the mind for the future: when creating the model for a level, the user cannot be given access to the Z3 solver object itself (otherwise, they could add constraints like `x == 0`, rendering the model useless). If we were using an ILP solver, this would make it impossible for the user to actually do anything. With Z3, I think they should still be able to solve many problems, due to constructs like `If` and logical connectives (`And`, `Or` ...). However, should this become limiting in the future, maybe there is a way to check that the user does not create any constraints on the input coordinates & movement vector.
...
- [ ] Introduction what requisites are needed to play this game and where to find docu i.e. for numpy, z3
