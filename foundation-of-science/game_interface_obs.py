...
def _():
    ...
    allowed_levels = {"NObservation", "Observation"}
    level_name = url_params.get("level")
    if level_name not in allowed_levels:
        level_name = "NObservation"

    currentLevel = getattr(gb, level_name)
    
    # import game_backend as gb
    # currentLevel = gb.NObservation
    return (currentLevel,)
...
def _(currentLevel, mo):
    ...
    get_observation, set_observation = mo.state([])
...
def _(
    ...
    observe_on_move = mo.ui.checkbox(label="Observe on move", value=False)

    def _do_observe(lvl):
        if lvl.observe():
            l = get_observation()
            l.append(lvl.position.copy())
            set_observation(l)
    ...
    def move_btn_click(value):
        ...
        if len(move_vec) == curr_lvl.dim_move:
            ...
            if observe_on_move.value:
                _do_observe(curr_lvl)
            set_lvl(curr_lvl)

    def observe_btn_click(value):
        curr_lvl = get_lvl()
        _do_observe(curr_lvl)
        set_lvl(curr_lvl)
    ...
    observe_btn = mo.ui.button(label="Observe now", on_click=observe_btn_click)
...
def _(
    ...
    mv_stack = mo.vstack(
        [*move_inputs, observe_on_move, move_btn, observe_btn],
        align="start",
        justify="space-between"
    )
    save_stack = mo.vstack([save_name, save_btn], align="start")
    reset_stack = mo.vstack([save_stack, dist_to_pnt_md, reset_lvl_btn, reset_plot_btn], align="start")

    mo.vstack(
        [
            position,
            mo.hstack([mv_stack, reset_stack], gap=3)
        ],
        align="center"
    )
    return
...
def _(get_history, get_lvl, get_observation, np):
...
def _(mo):
    ...
    <details><summary>Quick reference</summary>
     <ul class="acc-list">
          ...
          <li>Observe manually: <code>lvl.observe()</code> (or use the button)</li>
